...
public class LogAspect {
    @Before(value = "within(com.example.bookstore.service.*)")
    public void createStartLog(JoinPoint joinPoint) {
        log.info(joinPoint.getSignature() + " started.");
    }

    @After(value = "execution(public * com.example.bookstore.service.*.*(Long))")
    public void createEndLog(JoinPoint joinPoint) {
        log.info(joinPoint.getSignature() + " end.");
    }

    @AfterReturning(value = "within(com.example.bookstore.service.*)", returning = "retValue")
    public void logReturnValue(Object retValue) {
        log.info("Return value: " + retValue);
    }

    @AfterThrowing(value = "within(com.example.bookstore.service.*)", throwing = "throwValue")
    public void logException(Throwable throwValue) {
        log.error(String.valueOf(throwValue));
    }

    @Around(value = "@annotation(com.example.bookstore.aop.TimeAop)")
